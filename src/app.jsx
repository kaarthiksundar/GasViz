...
export function App() {
  ...
  async function handlePlot() {
    ...
    try {
      ...
      console.error(e);
      setError(
        'Failed to read/parse one or more files. Ensure they are valid JSON.'
      );
    }
  }
  ...
  function drawGeoLines(geo) {
    ...
    const iterable = Array.isArray(source)
      ? source.map((item, idx) => [
          item && (item.id ?? item.pipeId ?? idx),
          item && (item.geometry ?? item),
        ])
    ...
    for (const [pipeId, polyline] of iterable) {
      ...
      const addHotline = (pts) => {
        ...
        if (mid) {
          const tip = L.circleMarker([mid[0], mid[1]], {
            radius: 1,
            opacity: 0,
            fillOpacity: 0,
          })
    ...
    <Layout title={title} footer={null}>
      <section style={{ margin: '0 0 1rem 1rem' }}>
        <FileInput
          ...
          onSelect={(files) => setGeoFile(files[0] || null)}
        />
        <small>
          {geoFile ? `Selected: ${geoFile.name}` : 'No file selected'}
        </small>
        <br />
        <FileInput
          id="network"
          label="Network JSON"
          accept=".json,application/json"
          onSelect={(files) => setNetworkFile(files[0] || null)}
        />
        <small>
          {networkFile ? `Selected: ${networkFile.name}` : 'No file selected'}
        </small>
        <br />
        <FileInput
          id="solution"
          label="Solution JSON"
          accept=".json,application/json"
          onSelect={(files) => setSolutionFile(files[0] || null)}
        />
        <small>
          {solutionFile ? `Selected: ${solutionFile.name}` : 'No file selected'}
        </small>
        <div style={{ marginTop: '0.75rem' }}>
          <button
            onClick={handlePlot}
            disabled={!geoFile || !networkFile || !solutionFile}
          >
            Plot
          </button>
        </div>
